...
def weatherImpact(masterDict, world):
    ...
    while rounds < 1 or rounds > (masterDict['simRules']['maxTurns'] - masterDict['simRules']['Turn'])-1:
    ...
    while i < rounds:
        ...
        updateHistory(masterDict, humanPop, animalPop, pestPop, birthRate, deathRate, masterDict['simRules']['Turn'], temp, rainfall, masterDict['simRules']['maxTurns'], world, worldCats[1])
        i+=1
...
def updateHistory(masterDict, humanPop, animalPop, pestPop, birthRate, deathRate, turn, temperature, rainfall, maxTurns, world, animalName):
    historyFile = "history.txt"
    saveFile = "state.txt"
    with open(historyFile, 'a') as f:
        f.write(f"{str(masterDict)}\n")
    with open(saveFile, 'w') as f:
        f.writelines(f"World:{world}\nhumans:{humanPop}\n{animalName}:{animalPop}\npests:{pestPop}\nbirthRate:{birthRate}\ndeathRate:{deathRate}\nTurn:{turn}\nRainfall_cm:{rainfall}\nHeat_C:{temperature}\nmaxTurns:{maxTurns}")
